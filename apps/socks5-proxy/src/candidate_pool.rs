...
//! ```
//!
//! Dedup key: `(transport, addr, fingerprint)` plus the webtunnel endpoint
//! identity (`None` for non-webtunnel) — consistent with the fetcher's dedup key.
...
/// fingerprint)` plus the webtunnel endpoint identity (`None` for
/// non-webtunnel), consistent with the fetcher's dedup key.
pub type Key = (
    Option<String>,
    SocketAddr,
    Option<String>,
