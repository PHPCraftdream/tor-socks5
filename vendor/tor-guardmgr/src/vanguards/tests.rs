...
fn select_vanguards() {
    MockRuntime::test_with_various(|rt| async move {
        ...
        let selector = RelaySelector::new(tor_relay_selection::RelayUsage::vanguard(), exclusion);
...
fn full_vanguards_persistence() {
    MockRuntime::test_with_various(|rt| async move {
        ...
        let netdir = construct_custom_netdir_with_params(|_, _, _| {}, ENABLE_LITE_VANGUARDS, None)
            .unwrap()
            .unwrap_if_sufficient()
            .unwrap();
        let netdir_provider = vanguardmgr.init_vanguard_sets(&netdir).await.unwrap();
...
fn load_from_state_file() {
    MockRuntime::test_with_various(|rt| async move {
        ...
        let vanguardmgr = Arc::new(VanguardMgr::new(&config, rt.clone(), statemgr, false).unwrap());
