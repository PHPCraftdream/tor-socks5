...
impl BridgeStore {
    ...
    ///
    /// The temp file is created through [`persist_lock::TempFileGuard`]: its
    /// name embeds pid + a monotonic counter for uniqueness (concurrent saves
    /// -- within this process or from other processes, e.g. a CLI subcommand
    /// -- never share a temp file), and the guard holds an advisory
    /// exclusive lock on the temp for the whole create→rename span, so a
    /// reader's cleanup only deletes a temp whose owner provably died (the
    /// kernel releases the lock on death). The final rename is atomic:
    /// readers see either the old or the new full file, never a torn state.
    /// Whole-file rename means two concurrent writers resolve as
    ...
    pub(super) fn temp_path(&self, seq: u64) -> PathBuf {
        ...
        persist_lock::parent_dir(&self.path).join(persist_lock::temp_file_name(
            &file_name,
            std::process::id(),
            seq,
        ))
