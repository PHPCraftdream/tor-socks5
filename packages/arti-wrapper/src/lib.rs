...
pub struct Settings {
    ...
    /// Additional protocols supported by `pt_binary`, for probing inactive transports.
    pub extra_pt_protocols: Vec<String>,
...
impl Settings {
    pub fn is_default(&self) -> bool {
        self.bridges.is_empty()
            ...
            && self.extra_pt_protocols.is_empty()
...
fn build_config(settings: &Settings) -> Result<TorClientConfig> {
    ...
    if !settings.bridges.is_empty() {
        // Fast obfs4 history must not impose sub-second budgets on a slower bridge.
        builder
            .override_net_params()
        ...
        let transports: BTreeSet<&str> = settings
            ...
            .chain(settings.extra_pt_protocols.iter().map(String::as_str))
            .collect();
